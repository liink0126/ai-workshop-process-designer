...
# 3. GitHub에 푸시 (필수) - Vercel이 자동으로 배포 시작!
git push
```
...
1. **`git add .`** 
   - 수정한 모든 파일을 "스테이징 영역"에 추가
   - "이 파일들을 커밋할 거야"라고 Git에 알려주는 단계

2. **`git commit -m "메시지"`**
   - 변경사항을 하나의 "스냅샷"으로 저장
   - 메시지는 무엇을 수정했는지 간단히 설명 (예: "로딩 모달 개선", "워크숍 삭제 기능 추가")

3. **`git push`**
   - GitHub에 업로드
   - Vercel이 자동으로 감지하고 배포 시작! (약 1-2분 소요)
...
> 💡 **팁**: 
> - `git status`는 확인용이므로 생략 가능합니다
> - 실제로는 **3개 명령어만** 입력하면 됩니다
> - `git push` 후 Vercel 대시보드에서 배포 진행 상황을 확인할 수 있습니다
